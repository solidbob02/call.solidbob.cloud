...
이 문서는 앞으로 Claude Code가 CallGuard 실제 코드베이스(현재는 기획서 Jekyll 사이트만 존재)를 스캐폴딩·구현할 때 따라야 할 규칙이다. 기획서 본문([개발목차](/toc/))에 이미 정의된 내용과 모순되면 본문이 우선한다 — 이 문서는 그 내용을 "코드 생성 규칙"으로 번역한 것이다. **단, `_project/decisions/`(결정 기록)가 본문보다 우선한다**(`CLAUDE.md` §1 문서 우선순위, 2026-08-27 추가) — 기획서를 고치는 공식 경로가 결정 기록이기 때문이다. 이 기획서 자체가 A~G 기능 ID 체계를 갖고 있으므로, 별도 접두어(SFR 등)를 새로 만들지 않고 **기획서의 기능 ID를 그대로 요구사항 ID로 쓴다.**

> **`X-` 접두어 — 미결 후보 표기용 (2026-08-27 추가, `_project/decisions/022` ③).**
> ⚠ **위 문장의 취지를 오해하지 말 것.** 여기서 막는 것은 **`SFR` 같은 별도 ID *체계*를 두 벌
> 만드는 것**이지 새 기능 자체가 아니다. **새 기능이 필요하면 기획서에 새 ID(`B-7` 등)를 추가하면
> 된다** — 그 경로는 결정 기록이다(`CLAUDE.md` §1 문서 우선순위).
>
> 2026-08-27 "상담 중 고객에게 자료 발송" 논의가 여기서 멈췄던 실제 원인은 이 규칙이 아니라
> **`_project/plan.md` 가 그때까지 "수정하지 않는 사본"이어서 ID 를 추가할 방법이 없었던 것**이다.
>
> **`X-` 는 그 위의 편의 표기다** — 아직 채택 여부를 논의 중인 후보를 [미결 항목](/open-items/)에
> 적을 때 `X-1`·`X-2` 로 부른다. 채택되면 기획서에서 정식 ID 를 받고 `X-` 는 버린다.
> **코드에는 쓰지 않는다**(아래 §1.3).
...
3. **[추적성]** 생성하는 모든 코드/테스트/설정 파일 상단에 관련 기능 ID를 주석으로 명시한다. 예: `# Requirement: C-5`, `# Requirement: F-2`.
   **`X-` 접두어는 여기 쓰지 않는다** — 미결 후보 전용이므로, 채택돼 정식 ID 를 받은 뒤에 코드로 옮긴다.
...
> ⚠ **`A-5` 다, `A-3` 이 아니다.** 기획서의 `A-3` 은 「브라우저 실시간 전달(WebSocket)」,
> `A-4` 는 발화 구간 검출이다. `decisions/201` 초안의 `A-3` 표기는 오기이며 2026-08-28 정정됐다.
> **코드 주석에는 `# Requirement: A-5` 를 쓴다.**
| **C-6** | **콜 가드** — 고객 욕설·폭언 탐지 | (미구현) | 재현율 우선(C-1~C-4 와 같은 기준) | `_project/decisions/201` |
