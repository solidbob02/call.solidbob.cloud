...
> **예외 — `X-` 접두어 (2026-08-27 추가, `_project/decisions/022` ③).** 위 문장은 접두어 난립을 막으려는
> 것인데, **기획서에 없는 아이디어는 붙일 ID 가 없어 제안 자체가 막히는 부작용**이 있었다
> (2026-08-27 "상담 중 고객에게 자료 발송" 논의가 실제로 여기서 멈췄다).
> **`X-` 를 [미결 항목](/open-items/) 전용으로 연다** — `X-1`·`X-2` 는 아직 채택되지 않은 후보를 가리키고,
> 채택되면 기획서에서 정식 ID 를 받으며 `X-` 는 버린다. **코드에는 쓰지 않는다**(아래 §1.3).
