...
오른쪽 패널 상단은 칩 두 개(콘텐츠 보기 · 도메인)다. 「팝업창」은 카드 본문만,
「충족요건」은 종결 체크리스트만 보여 준다. 종결 모달은 쓰지 않는다.
...
- 카드가 `closure_type`과 연결된 경우 「충족요건」 뷰에 체크리스트를 보여 준다.
  연결되지 않았으면 「충족요건 없음」. 근거가 전부 충족되면 「종결 처리」 버튼이
  나타난다(충족요건 뷰에만). 미충족이면 버튼을 두지 않거나 비활성.
