...
**완료 조건**: 4인 승인 후 마이그레이션 확정. 인터페이스 계약과 같은 회의에서 정한다.
**2026-08-26 갱신**: 도메인 정리까지 반영된 스키마를 팀원 확인으로 승인 처리했다(아래
참고). 실제 마이그레이션 적용은 아직 미착수 — 이후 남는 승인 대기 항목들은 즉시
개별 승인 대신 [4주차 말 전체 검증 체크포인트](/docs/08/)에서 처리한다 —
`_project/decisions/008`.
