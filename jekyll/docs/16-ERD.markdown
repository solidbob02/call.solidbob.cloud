...
개체(가입자·문서·후속조치·공백 리포트)가 더 필요하다. 그래서 실제로는 **17개 테이블**로
설계했다.
...
|---|---|---|
| `call` | D-1(상담 요약)·D-2(문의 유형)는 통화당 값이 하나(1:1) — `call`에 병합 | `call` |
| (없음) | F-3(반복 문의 연결)이 "동일 고객" 식별자 필요, TERM-5.3(명의변경 제한)이 가입자 상태(체납·분실신고) 확인 필요 | `subscriber`, `plan` |
...
| `closure` | 그대로 두되 evidence 필드를 표로 역정규화(아래) | `closure` |
...
**팀원에게 전달한 피드백**: 팀원 설계엔 `subscriber`/`plan`이 없어 [F-3](/docs/02/)(반복
문의 연결)·[TERM-5.3](/docs/02/)(명의변경 제한)을 구현할 수 없고, `follow_up_action`·
`knowledge_gap`(D-3·D-4)이 통째로 빠져 있으며, `eval_result`에 절대 규칙 위반 플래그가
없다.
...
부모가 참조·분류 대상일 뿐 자식이 독립적 정체성을 가지는 관계(`subscriber`→`plan`,
...
- **3NF**: `subscriber`는 `plan_name`을 직접 갖지 않고 `plan_code`로 `plan`을 참조한다.
  `recommendation_card`도 문서 제목을 복사하지 않고 `document_id`로 `document`를 참조한다.

## 의도적으로 역정규화한 것

컬럼이 3~4개뿐인 "너무 좁은" 테이블은 두 곳에서 부모에 병합했다.

1. **`closure_evidence`를 안 만들고 `closure`에 evidence 컬럼 7개를 직접 뒀다.** 처리
   유형(해지/명의변경/보상)마다 확인 항목이 달라 교과서적으로는 `(closure_id, field_name,
   is_satisfied)` 3컬럼 EAV 테이블이 "더 정규화된" 형태지만, 필드가 7개로 고정돼 있고
   [F-2 게이트](/docs/02/)가 "이 필드들이 전부 true인가"를 한 행에서 바로 봐야 한다.
   EAV로 쪼개면 매번 재조립해야 해서 실용성이 떨어진다.
...
1. **가입자 → 통화**: `subscriber` 1명이 `call`을 여러 번. `plan`은 참조만 하고 복사 안 함.
   `call`은 `agent`(상담원)도 FK로 참조 — 상담원별 통계 노출은 하지 않는 걸 전제로 함
   ([부록 B](/docs/13/)).
...
4. **통화 → 종결**: `call` 1건에 `closure` 시도가 여러 번(거절되면 재시도) —
   **UPDATE 없이 INSERT만 하는 append-only** ([F-4](/docs/02/)). `document` 근거 인용도 동일.
...
- [ ] `subscriber_id`를 해시/난수로 할지, 실명 연동이 필요한지 (정성윤)
- [ ] `closure`의 evidence 7컬럼 vs 팀원 안(EAV+정의테이블) — F-2 구현 시 재검토 (류준)
- [ ] 팀원 ERD에 없는 `subscriber`/`plan`·`follow_up_action`·`knowledge_gap` 병합 논의
- [ ] `resource_center`를 G-2 착수 확정 전까지 실제 마이그레이션에서 뺄지
- [ ] `document.document_id`(`TERM-3.2` 등)가 `knowledge-base/`의 `<!-- id: -->` 주석과
      전부 일치하는지
