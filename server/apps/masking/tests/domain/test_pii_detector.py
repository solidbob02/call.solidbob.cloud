...
# ── 같은 글자를 두 이름으로 부르는 경우 (2026-08-27) ─────────────────────────

@pytest.mark.parametrize("text, expected", [
    ("이체할 계좌번호는 110-1234-567890 이에요", "P3"),   # 13자리 — 계좌 문맥
    ("본인확인 할게요 주민번호 1234561234567 입니다", "P1"),  # 13자리 — 주민 문맥
    ("송금할 계좌 1234561234567 입니다", "P3"),
    ("주민등록번호 110-1234-567890 맞습니다", "P1"),
])
def test_자릿수가_같으면_문맥으로_패턴을_가른다(text, expected):
    """13자리는 주민등록번호(P1)이면서 계좌번호(P3, 10~14) 범위이기도 하다.
    글자는 어느 쪽으로 잡아도 가려지지만 **라벨이 틀리면 화면이 거짓말한다.**"""
    assert _patterns(text) == [expected]


def test_문맥이_없으면_규칙_순서로_하나만_남긴다():
    """둘 다 남기면 화면에 배지가 두 개 뜬다. 판단할 근거가 없을 때 라벨을 늘리지 않는다 —
    가리는 것은 어차피 같으므로 개인정보가 새지는 않는다."""
    assert len(_patterns("번호 1234561234567 입니다")) == 1
