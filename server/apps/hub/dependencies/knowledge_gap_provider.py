...
"""KnowledgeGapPort 프로바이더. MySQL 이 없으면 501 — 신고를 받아놓고 버리지 않는다.
...
from hub.adapter.outbound.mysql.knowledge_gap_repository import MySqlKnowledgeGapRepository
...
def get_knowledge_gap_port(request: Request) -> KnowledgeGapPort:
    ...
    if not settings.mysql_configured:
        raise HTTPException(
            ...
            detail="MySQL 이 설정되지 않았습니다 — 신고를 저장할 곳이 없습니다 (infra/README.md)",
        )
    return MySqlKnowledgeGapRepository(build_connection_factory(settings))
