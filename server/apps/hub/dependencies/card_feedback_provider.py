...
"""CardFeedbackPort 프로바이더. MySQL 이 없으면 501 — 받아놓고 버리지 않는다."""

from __future__ import annotations

from fastapi import Depends, HTTPException, Request, status

from hub.adapter.outbound.mysql.card_feedback_repository import MySqlCardFeedbackRepository
...
def get_card_feedback_port(request: Request) -> CardFeedbackPort:
    ...
    if not settings.mysql_configured:
        raise HTTPException(
            ...
            detail="MySQL 이 설정되지 않았습니다 — 피드백을 저장할 곳이 없습니다 (infra/README.md)",
        )
    return MySqlCardFeedbackRepository(build_connection_factory(settings))
