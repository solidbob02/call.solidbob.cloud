...
"""TranscriptQueryPort 프로바이더. **MySQL 설정이 없으면 501** 이다.
...
from hub.adapter.outbound.mysql.transcript_query_repository import MySqlTranscriptQueryRepository
...
def get_transcript_query_port(request: Request) -> TranscriptQueryPort:
    ...
    if not settings.mysql_configured:
        raise HTTPException(
            ...
            detail="MySQL 이 설정되지 않았습니다 — infra/README.md 참고",
        )
    return MySqlTranscriptQueryRepository(build_connection_factory(settings))
