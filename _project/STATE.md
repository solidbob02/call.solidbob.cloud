...
**최종 갱신**: 2026-08-27 (frontend 대시보드 셸을 패널별 헤더로 분리 · 플랫폼 히어로 아래 카피 보강 · 디자인 토큰 통일·자막 스크롤백·검색/수동 검색/통화 후 처리/카드 채택 기록·도커 infra/ 단일화 + ai 병합 — 류준 님 w2-naive-rag 완료로 첫 실측 Recall@5 0.857/MRR 0.702 확보, server는 규칙 완화 제안 후 팀 회신 대기)
...
프론트: `apps/dashboard` 자막 | 이용약관·충족요건. 오른쪽 패널 상단은
`ArrowSelectChip` 두 개(충족요건/팝업창 · 도메인). `apps/platform`
랜딩(포트 3000)은 밝은 목업 구조(헤더→히어로 `100dvh`→놓치는 순간→하는 일 3분할→도메인 4→푸터,
섹션 padding `88px 56px`, 키워드 2.8초 순환). 티켓 `w1-platform-landing` 은 `in-progress`.
`reference/mockup.html` 은 저장소에 없어 요청 스펙+대시보드 토큰으로 맞췄다.
`frontend`→`main` PR은 아직 없다. 고객 화면은 009로 철회.
