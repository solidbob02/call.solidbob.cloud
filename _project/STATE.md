...
**최종 갱신**: 2026-08-26 (`server/`·`ai/` 분리 + frontend 대시보드; Jekyll 링크 CI 수정)
...
프론트: `apps/dashboard` 자막 | 이용약관·충족요건. 고객 화면은 009로 철회.
...
`apps/dashboard` 자막 | 이용약관·충족요건. F-2는 카드 안 체크리스트, approved만 「종결 처리」.
...
PostgreSQL 마이그레이션은 2026-08-27 도커로 적용 완료(원래 MySQL 이었고 같은 날 전환 — decisions/016).
