...
**브랜치 통합 — 하지 않기로 결정.** 네 브랜치 체제를 유지한다. 나도 같은 날 같은 결론에 도달했으나
정성윤 님이 `main` 보호 설정 실제 적용까지 마쳐 그쪽 기록이 정본이다(`progress` 2026-08-26 (16)).
`ai` 브랜치에 고유 커밋 0건이라는 확인은 그대로 유효하다.

**브랜치 관련 사고 기록**: "backend 브랜치 가져와줘" 를 브랜치 전환으로 읽고 `checkout backend`
→ `merge ai` → `push origin backend` 까지 진행했다가 사용자가 중단시켰다. 로컬은 되돌렸지만
`origin/backend` 는 이미 `500826b` 로 움직였고, 류준 님 브랜치라 force push 대신 **그대로 두기로**
했다(사용자 결정). 내용은 `ai` 것과 같고 fast-forward 라 히스토리는 꼬이지 않았다.

**다음 세션이 확인할 것**: `w2-naive-rag` 는 **ES 인덱스 분할 결정이 나야** 착수할 수 있다 —
현재 2주차 진행을 막고 있는 유일한 항목이다.
**주의**: `server/`·`ai/` 분리로 `scripts/index_knowledge_base.py` 의 `sys.path` 가
`fastapi/apps` 를 가리키고 있어 깨졌을 수 있다 — 실행해서 확인할 것. 충돌 대응은 브랜치가 아니라 **티켓 선점**
(착수 시 `status: in-progress` 로 먼저 푸시)으로 간다.
