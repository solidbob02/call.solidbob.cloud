...
| # | 무엇이 막았나 | 어떻게 고쳤나 |
|---|---|---|
| ① | F·G·H·I 동결에 **해제 조건이 없었다**(판정에 하네스 수치가 필요한데 전부 「측정 불가」) | 해제 조건 2개 명문화 (`CLAUDE.md §1`) |
...
**⚠ `022` 원안 ②(「절반 이상」)는 쓰지 않았다.** PR #36 으로 트리거 구현이 들어온 뒤라,
원안대로면 **트리거 배선 하나로 2/4 = 절반이 되어 자동 해제**된다. 그 시점에도
`ai/apps/compliance/`·`server/apps/postcall/` 은 **디렉터리 자체가 없다.**
→ 「**전부** 501 을 벗어나고 **6.1절 미달(❌) 0건**」으로 좁혔다. **지금 기준으로는 해제 안 된다**
(B-0 `0.857 ❌`, 코어 포트 4개 중 3개 501). 판정이 가능해진 것이지 통과한 게 아니다.

**②는 하네스 출력으로 판정한다 — 결재 절차를 두지 않았다.** 승인 자리를 만들면 그 자리가
다시 병목이 된다. 이 문서가 없애려는 것과 같은 종류의 문제다.
...
**미결 2건 추가**: ① B-0 표본 설계(동결 해제 조건 ②에 직접 물림) ② 배포 한 컨테이너 여부
(`ai/` 는 HTTP 표면·`fastapi` 의존성이 없는 **라이브러리**인데 `§3` 은 `ai.solidbob.cloud` 를 적고 있다).
