...
**최종 갱신**: 2026-08-27 (**`/hub/search` 가 실제로 검색한다** — 501 을 벗어난 첫 검색 경로. 평가 DB 기록·과잉 마스킹 측정도 완료. `server/` 는 모델 없이 할 일이 끝났다)
...
## ▶ 다음 세션이 가장 먼저 볼 것 (2026-08-27 최종)
...
| **정성윤** | **배포를 한 컨테이너로 할지** (`decisions/023` — 같은 프로세스로 갔는데 `CLAUDE.md` 는 두 도메인) |
...
**`023` 까지 찼다. 다음은 `024`.** 하루에 **세 번** 번호가 겹쳤다 —
