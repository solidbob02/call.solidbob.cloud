...
**브랜치 통합 — 하지 않기로 결정 (2026-08-26)**. 네 브랜치 체제를 그대로 유지한다. 검토 중 확인된 사실:
- `ai` 브랜치에 고유 커밋 **0건** — 모든 작업이 `main` 에 반영됐다. 지금 `ai` 를 지워도 잃는 게 없다
- ~~합친 뒤 `test.yml:19` 와 `CLAUDE.md` §7 을 고쳐야 한다~~ → **통합을 안 하므로 둘 다 그대로 둔다**
...
현재 2주차 진행을 막고 있는 유일한 항목이다. 충돌 대응은 브랜치가 아니라 **티켓 선점**
