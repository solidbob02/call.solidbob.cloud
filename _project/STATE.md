...
**2주차** / 8스프린트 (2026-08-20 ~ 2026-10-27). 1주차 목표 6개 전부 달성.
...
| `apps/` | 조서희 | 대시보드: 다산콜센터 mock 6종(A-5 5언어 + 한국어 C-5 마스킹). 상담기록 전환 시 자막·카드 함께 교체. 랜딩 `apps/platform` → [www.solidbob.cloud](https://www.solidbob.cloud/) (Vercel) |
...
| **정성윤** | 배포를 한 컨테이너로 할지 (`decisions/024` — 같은 프로세스인데 `CLAUDE.md` 는 두 도메인) |
...
**`024` 까지 찼다. 다음부터 담당자별 번호대** — 정성윤 `1xx` · 류준 `2xx` · 장민석 `3xx` ·
...
- 원격 인증: `gh` = SeongYuna (협업자, push 가능 / admin 아님). 저장소 설정 변경은 `solidbob02` 만.
