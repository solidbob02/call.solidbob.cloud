...
| ② | 기획서에 없는 아이디어는 **붙일 ID 가 없어 코드를 못 썼다**(§1 접두어 금지 + §1.3 ID 주석 필수) | `X-` 접두어를 미결 전용으로 개방 |
| ③ | `decisions/012` 「경계에 걸친 작업」에 **정의가 없었다**(08-27 6건 정지) | 소유/배선 분리 명문화 |
| ④ | 도메인 4종 조항이 **"예시로도"** 까지 막았다 | 해당 표현 삭제 |
