...
**기간**: 8주 (2026-08-24 착수)
**원본 기획서**: `_project/plan.md` — 판단이 갈릴 때의 최종 근거. 기획서와 다르게 갈 때는 반드시 `_project/decisions/` 에 이유를 남긴다.
...
| C | 실시간 컴플라이언스 감지 | 후순위 |
| D | 통화 후 처리 (요약·유형분류·지식베이스 공백 리포트) | 후순위 |
| **E** | **평가 하네스 (골든셋·자동채점·CI)** | **사수** |

범위가 넘칠 때 버리는 순서는 **D → C**. B 와 E 는 어떤 경우에도 사수한다.
...
**대표 실험**: STT 오류율 0~20% 구간별 Recall@5 곡선 (BM25 / 벡터 / 하이브리드). 이 곡선 하나가 프로젝트 전체를 설명한다.
...
| 주차 | 주제 | 이 주차의 산출물 |
|---|---|---|
| 1 | 기반 + 평가셋 | AI Hub 신청, 인터페이스 스키마 확정, 골든셋 50건, 평가 하네스 골격 |
| 2 | 베이스라인 | 가장 단순한 RAG → **기준선 숫자 기록** |
| 3 | 실시간화 | 스트리밍 STT + WebSocket + 트리거 v1 + 자막 화면 |
| 4 | 검색 품질 | nori + dense_vector + RRF, 청킹 3종 비교 |
| 5 | **오류 내성 (핵심)** | 오류율별 성능 곡선, 자모 필터·리랭커 재측정 |
| 6 | 생성 + 컴플라이언스 | 근거 기반 카드, 출처 표시, 분류기 파인튜닝 |
| 7 | 운영 관점 | p95 레이턴시, 캐싱, 토큰 비용, 공백 리포트, Docker |
| 8 | 마감 | 아키텍처 다이어그램, **실패 사례 분석**, 성능 리포트, 발표 |

**1주차 최우선**: AI Hub 데이터 신청 (휴대폰 인증 + 승인 대기). 늦어지면 5주차 핵심 실험이 통째로 밀린다.
