...
- **한 줄**: 서울시 120 다산콜센터 상담에서, 통화를 실시간으로 들으며 **그 절차에 필요한 구비서류를
  상담원 화면에 자동으로 띄우고**, 외국인 고객의 발화를 **한글로 동시 통번역**해 상담원이 이해하게 하며,
  컴플라이언스 위반·고객 폭언·개인정보를 실시간으로 탐지·마스킹한다.
- 2026-08-21~24에는 같은 저장소가 AdPass(AI 광고 규제 사전검수) 기획서를 다루었고, 08-25 rev.4 반영으로 CallGuard로 전환됐다.
- **도메인 (2026-08-28 단일화)**: **다산콜센터(서울시 120) 하나**다. 금융보험·쇼핑·질병관리본부는
  지식베이스·골든셋·코드 분기에서 **전부 삭제**했다.
  다산을 고른 이유는 실측이다 — ① **음성 데이터를 가진 유일한 도메인**(다산콜DB 6,614건·12.7시간)
  ② 서류 문의 상위 단어에 **`여권` 이 4위**로 있어 외국인 맥락이 데이터에 이미 있다(금융에는 없다)
  ③ 120이 **사람 통역으로 외국어 상담을 이미 운영 중**이라 수요가 검증돼 있다.
  근거·되돌리는 법: `_project/decisions/201-다산콜센터-단일-도메인-외국인-지원-전환.md`
  (`decisions/004` 4종 지원을 철회한다).
- **누구를 돕는가 (2026-08-28 추가)**: 국내 체류외국인 **287만 명**(2026-06-30, 정부는 연내 300만 전망) —
  외국인 노동자·재외국인·교포·결혼이민자가 **한국어가 어눌한 채로** 민원 전화를 건다.
  AICC 시장에 대기업이 가득한 상황(`jekyll/docs/09`)에서 **차별점을 여기서 만든다.**
...
> ⚠ **정본은 `_project/plan.md`(rev.5)다.** 사이트 문서(`jekyll/docs/01~16`)는 5인 트랙
> 버전(`plan-rev4-alt-5인안.md`)을 바탕으로 작성됐고 `plan.md`는 3인 실명 버전이라 값이
> 갈렸었다. **대표적이던 트리거 허용 창(800 vs 1,500ms)은 2026-08-28 에 1,500ms 로
> 정리했다**(`decisions/202` — 새 결정이 아니라 2026-08-25 팀 확정의 누락된 반영).
> 다른 값이 또 갈리면 **결정 기록을 먼저 본다** — 이 건은 기획서 쪽이 낡아서, "파생 문서가
> 기획서와 다르면 기획서가 맞다"를 기계적으로 적용했다면 틀린 값을 살렸을 것이다.
...
| 블록 | 내용 | 우선순위 |
|---|---|---|
| A-1~A-4 | 실시간 STT (스트리밍·화자분리·브라우저 전달·발화구간 검출) | 필수 |
| **A-5** | **동시 통번역** — 고객 외국어 → 한글(상담원 화면) · 상담원 한국어 → 고객 모국어 **TTS** | **사수 — 차별점** |
| **B** | **필요서류 자동 제시** — 대화 맥락으로 절차를 판정해 구비서류를 빠짐없이 띄운다 (RAG 핵심) | **사수 — 메인** |
| C-1~C-4 | 실시간 컴플라이언스 감지 (**상담원** 발화 위반) | 필수 |
| **C-5** | **개인정보 실시간 마스킹** (자막·저장 양쪽 앞단) | **사수 — 코어** |
| **C-6** | **콜 가드** — **고객** 욕설·폭언 탐지, 상담원 보호 | 필수 |
| D | 통화 후 처리 — 요약·유형분류·공백 리포트 + **감정분석 기반 상담품질 평가** | 필수 |
| **E** | **평가 하네스** (골든셋·자동채점·CI) | **사수** |
| F-2 | 종결 요건 게이트 → **필요서류 체크리스트로 전용** | B 의 판정부 |
| 에스컬레이션 / G-2 / H·I | 확장 모듈 | 여유 시 |

> **2026-08-28 재편**(`_project/decisions/201`). 바뀐 것 넷 —
> ① **B 가 "문서 추천"에서 "필요서류 제시"로 좁혀졌다.** 실측 근거: 다산 고객 질문의 2.60%(174건)가
> 서류를 묻고, 그 문의가 **69종 서비스**에 흩어져 있다 — 상담원이 외울 수 있는 규모가 아니다.
> ② **A-5 동시 통번역이 새 차별점**이다(⚠ `decisions/201` 초안의 `A-3` 은 오기 —
> 기획서의 A-3 은 WebSocket 전달이다. `plan.md` rev.5 요약 참고). ③ **C-6 콜 가드**를 더했다 — C-1~C-4 는 상담원 위반을,
> C-6 은 고객 폭언을 잡는다. **방향이 반대라 대체가 아니라 추가**다. ④ **F-2 를 필요서류
> 체크리스트로 전용**한다. 다산은 종결 처리 유형이 없지만(`POLICY-1`), "필수 항목을 규칙으로
> 정의하고 빠진 것을 `missing` 으로 돌려주는" 구조가 같다. 서류를 빠뜨리면 고객이 헛걸음하므로
> **재현율 우선**인 것도 같다.
>
> **에스컬레이션은 보류한다** — 제안된 그림이 AI 콜봇 선응대를 전제하는데 우리에게 콜봇이 없다.
...
5. **기준선 미달은 CI 실패.** 절대 규칙(C-5 누락 0건, F-2 판정 정확도)은 평균이 아니라 **1건이라도 뚫리면 실패**로 처리한다.
   **단, 통과는 잰 범위 안에서만 성립한다** — 표본이 없는 항목은 통과가 아니라 «측정 불가»다(2026-08-28).
   하네스가 그 둘을 구분해 보고한다(`NO_SAMPLES` — 「골든셋에 채점 대상이 없다」).
   **2026-08-28 현재 실제로 재고 있는 범위**: C-5 는 **P4·P6·P7 뿐**이고 P1·P2·P3·P5 는 표본이 0건이다
