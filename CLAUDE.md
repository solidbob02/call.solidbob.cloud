...
- **데모 도메인 (2026-08-26 확정)**: 가상 통신사 "한별텔레콤" 단일 시나리오 대신, 실제로 확보한 AI Hub
  ...
  문서·발표의 예시로 드는 것까지 막지는 않는다(2026-08-27 완화 — 원문은 "예시로도 새로 만들지
  않는다"였다). 근거·되돌리는 법:
...
**기획서에 없는 아이디어는 `X-` 접두어로 미결 항목에 올린다 (2026-08-27 추가, `decisions/022` ③).**
`.claude/rules/rfp-harness.md §1` 이 새 접두어를 금지해 **붙일 ID 가 없어 제안 자체가 막히던** 것을 연다
(2026-08-27 "상담 중 고객 자료 발송" 논의가 실제로 여기서 멈췄다). `X-1`·`X-2` 는 아직 채택되지 않은
후보를 가리키고, 채택되면 기획서에서 정식 ID 를 받으며 `X-` 는 버린다.
**`X-` 를 코드·테스트의 `# Requirement:` 주석에 쓰지 않는다** — 접두어 난립 방지 취지는 그대로 지킨다.
