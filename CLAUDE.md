...
> **해제 조건 (2026-08-27 추가 — `_project/decisions/022`)**
>
> 위 문장에는 원래 **끝나는 지점이 없었다.** "기준선 통과"를 판정하려면 평가 하네스 수치가
> 필요한데, 하네스가 전부 "측정 불가 — 모듈 미구현"으로 보고하던 동안에는 판정 자체가
> 불가능했다. 결과적으로 **해제 조건이 없는 영구 동결**이었고, 확장 아이디어가 논의 단계에서
> 멈춘 사례가 2026-08-27 에 실제로 나왔다.
>
> **아래 둘 중 하나면 해제한다 —**
...
>
> ②는 **사람의 승인이 아니라 하네스 출력으로 판정된다** — 별도 결재 절차를 두지 않는다.
> 해제 전이라도 **미결 항목에 후보로 올리는 것은 막지 않는다**(§4 `X-` 접두어).
